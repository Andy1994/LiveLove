//
//  GFMyFolloweeListViewController.m
//  GetFun
//
...
@interface GFMyFolloweeListViewController ()
...
@implementation GFMyFolloweeListViewController
...
//    }
    ...
    self.isLoading = YES;
    @weakify(self)
    [GFNetworkManager queryFolloweeListWithUserId:nil refTime:self.refTime success:^(NSUInteger taskId, NSInteger code, NSString *apiErrorMessage, NSArray<GFFollowerMTL *> *followeeList, NSNumber *refTime) {
        @strongify(self)
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.style = GFFollowTableViewCellStyleMyFollowee;
    GFFollowerMTL *followeeMTL = self.followeeList[indexPath.row];
    [cell bindWithModel:followeeMTL];
    cell.delegate = self;
    return cell;
}

- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [MobClick event:@"gf_gr_01_15_04_1"];
...
- (void)followActionWithButton:(UIButton *)button InCell:(GFFollowTableViewCell *)cell {
    ...
    switch ([followeeMTL followState]) {
        ...
        case GFFollowStateFollowing: {
            [MobClick event:@"gf_gr_01_15_01_1"];;
            break;
        }
            ...
            switch ([followeeMTL followState]) {
                ...
                case GFFollowStateFollowing: {
                    [UIAlertView bk_showAlertViewWithTitle:[NSString stringWithFormat:@"是否取消对%@的关注？", followeeMTL.user.nickName] message:nil cancelButtonTitle:@"否" otherButtonTitles:@[@"是"] handler:^(UIAlertView *alertView, NSInteger buttonIndex) {
                        if (buttonIndex==0) {
                            [MobClick event:@"gf_gr_01_15_03_1"];
                        } else{
                            [MobClick event:@"gf_gr_01_15_02_1"];
