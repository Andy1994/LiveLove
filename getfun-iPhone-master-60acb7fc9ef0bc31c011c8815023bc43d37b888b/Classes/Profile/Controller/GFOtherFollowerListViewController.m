//
//  GFOtherFollowerListViewController.m
//  GetFun
//
...
@interface GFOtherFollowerListViewController ()
...
@implementation GFOtherFollowerListViewController
...
//    }
    ...
    self.isLoading = YES;
    @weakify(self)
    [GFNetworkManager queryFollowerListWithUserId:self.userId refTime:self.refTime success:^(NSUInteger taskId, NSInteger code, NSString *apiErrorMessage, NSArray<GFFollowerMTL *> *followerList, NSNumber *refTime) {
        @strongify(self)
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.style = GFFollowTableViewCellStyleOtherFollower;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [MobClick event:@"gf_gr_01_14_05_1"];
...
- (void)followActionWithButton:(UIButton *)button InCell:(GFFollowTableViewCell *)cell {
    ...
    switch ([followerMTL followState]) {
        case GFFollowStateNo: {
            [MobClick event:@"gf_gr_01_14_01_1"];
            break;
        }
        case GFFollowStateFollowing: {
            [MobClick event:@"gf_gr_01_14_02_1"];;
            break;
        }
            ...
            switch ([followerMTL followState]) {
                ...
                case GFFollowStateFollowing: {
                    [UIAlertView bk_showAlertViewWithTitle:[NSString stringWithFormat:@"是否取消对%@的关注？", followerMTL.user.nickName] message:nil cancelButtonTitle:@"否" otherButtonTitles:@[@"是"] handler:^(UIAlertView *alertView, NSInteger buttonIndex) {
                        if (buttonIndex==0) {
                            [MobClick event:@"gf_gr_01_14_04_1"];
                        } else{
                            [MobClick event:@"gf_gr_01_14_03_1"];
